...
def rip_id3(session, track, outputdir): # write ID3 data
    ...
    comment = os.path.basename( outputdir )
    ...
    if coverFound:
        if args.oldtags:
            ...
            call(["eyeD3", "--add-image", "cover.jpg:FRONT_COVER", "-t", title, "-a", artist, "-A", album, "-n", str(num_track), "-Y", str(year), "--comment", comment, "-Q", filepath])
    else:
        if args.oldtags:
            call(["eyeD3", "--to-v2.3", "-t", title, "-a", artist, "-A", album, "-n", str(num_track), "-Y", str(year), "-Q", filepath])
        else:
            call(["eyeD3", "-t", title, "-a", artist, "-A", album, "-n", str(num_track), "-Y", str(year), "--comment", comment, "-Q", filepath])

    print(filepath + " written")
    # delete cover
    call(["rm", "-f", "cover.jpg"])
    # push file into itunes
    call(["cp", filepath, "/Users/nicodeceulaer/Music/iTunes/iTunes Media/Automatically Add to iTunes/"] )
